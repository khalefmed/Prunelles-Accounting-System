...
const UpdateType = ({name, debiteur, id}) => {
    const { i18n, t } = useTranslation();
    
        const [nom, setNom] = useState(name);
        const [is_debiteur, setIsDebiteur] = useState(debiteur);
      ...
      <AlertDialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[500px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
        ...
        <form onSubmit={(e) => creer(e)} className='w-full max-sm:w-full flex flex-col gap-6 mt-6  '>
                ...
                <div className='text-center'>
                  <input type="submit" onClick={creer} value={t('Modifier')}  className="w-1/2 rounded rounded-lg text-center py-2 mt-2 bg-buttonGradientSecondary  text-whiteColor font-normal cursor-pointer " />
                </div>
            </form>
